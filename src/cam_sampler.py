...
def get_patches(image_tensor: torch.Tensor, x: int, y: int, w: int, h: int,  size: tuple=(14,14),
  ...
  for i in range(w // size[0]):
    for j in range(h // size[1]):
      ...
      if mode == "small" or bounding_box_check(x_temp, y_temp, list_of_bounding_box_cords):
        ...
        # print(patch.shape, w_temp, h_temp)
        # print(y_temp, y_temp+h_temp, x_temp, x_temp+w_temp)
  ...
  return list_of_patches, list_of_patch_cords
...
def image_patching(image_tensor: torch.Tensor, model: nn.Module, cam, patch_size: Tuple[int, int]=(8, 8), bigger_grid_patch_size: Tuple[int, int]=(32, 32),
    ...
    assert bigger_grid_patch_size[0] % patch_size[0] == 0, "bigger patch size must be divisable by the smaller patch size"
    assert image_tensor.shape[-1] % bigger_grid_patch_size[0] == 0, "image height and width must be divisable by the patch size"

    rgb_img = image_tensor
    ...
    if len(cnts) == 0:
      bigger_patches, bigger_coords = get_patches(image_tensor, 0, 0, HW, HW, size=bigger_grid_patch_size, mode="big")
      patch_coords_tensor = torch.tensor(bigger_coords, dtype=torch.float32)
      return torch.stack(bigger_patches), patch_coords_tensor
    else:
      ...
      for c in cnts:
        ...
        if h > 0 and w > 0 and x < HW and y < HW:
            smaller_patches, smaller_coords = get_patches(image_tensor,x,y,w,h,size=patch_size)
            res.append(smaller_patches)
            patch_coords.extend(smaller_coords)
            list_of_bounding_box_cords.append((x,y,x+w,y+h))

      bigger_patches, bigger_coords = get_patches(image_tensor, 0, 0, HW, HW, size=bigger_grid_patch_size, \
                                   mode="big", list_of_bounding_box_cords=list_of_bounding_box_cords)

      res.append(bigger_patches)
      patch_coords.extend(bigger_coords)
      res = sum(res, [])

      # Convert patch coordinates to PyTorch tensor
      patch_coords_tensor = torch.tensor(patch_coords, dtype=torch.float32)

      return torch.stack(res), patch_coords_tensor